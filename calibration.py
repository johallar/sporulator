...
class StageCalibration:
    ...
    def __init__(self):
        self.known_standards = {
            "10um_scale_bar": {
                "length_um": 10,
                "description": "10 micrometer scale bar"
            },
            "20um_scale_bar": {
                "length_um": 20,
                "description": "20 micrometer scale bar"
            },
            "50um_scale_bar": {
                "length_um": 50,
                "description": "50 micrometer scale bar"
            },
            "100um_scale_bar": {
                "length_um": 100,
                "description": "100 micrometer scale bar"
            },
            "pollen_grain": {
                "length_um": 25,
                "description": "Average pollen grain (25μm)"
            },
            "red_blood_cell": {
                "length_um": 7.5,
                "description": "Human red blood cell (7.5μm)"
            },
        }
    ...
    def detect_scale_bar(self,
        ...
        for line in lines:
            ...
            if angle < 10 or angle > 170:
                ...
                position_score = (x1 + x2) / (2 * w) + (y1 + y2) / (
                    2 * h)  # Favor bottom-right

                quality_score = (length_score * 0.5 + angle_score * 0.3 +
                                 position_score * 0.2)
    ...
    def detect_micrometer_divisions(self,
                                    image,
                                    min_tick_length=25,
                                    max_tick_length=100):
        ...
        for line in lines:
            ...
            if (80 <= angle <= 100) and (min_tick_length <= length <=
                                         max_tick_length):
        ...
        if len(vertical_lines
               ) < 3:  # Need at least 3 tick marks to calculate spacing
            return None
        ...
        for i in range(1, len(vertical_lines)):
            spacing = vertical_lines[i]['center_x'] - vertical_lines[
                i - 1]['center_x']
        ...
        return {
            ...
            'quality_score': min(len(vertical_lines) / 10.0,
                                 1.0)  # Better with more tick marks
        }
        ...
        if reference_type == "scale_bar":
            ...
            if scale_bar and known_length_um:
                pixel_scale = self.calculate_pixel_scale(
                    scale_bar['length'], known_length_um)
                ...
                length_confidence = min(relative_length / 0.1,
                                        1.0)  # Good if >10% of image width
                confidence = (quality_confidence * 0.7 +
            ...
            if divisions and known_length_um:
                ...
                pixel_scale = self.calculate_pixel_scale(
                    divisions['spacing_pixels'], known_length_um)

                # Calculate confidence based on number of divisions and consistency
                quality_confidence = divisions.get('quality_score', 0.5)
                consistency_confidence = min(divisions['num_divisions'] / 10.0,
                                             1.0)  # Better with more divisions
                confidence = (quality_confidence * 0.6 + consistency_confidence
                              * 0.4) * 0.9  # High confidence for micrometer

                results.update({
                    'pixel_scale':
                    pixel_scale,
                    'detection_method':
                    'micrometer_divisions',
                    'confidence':
                    confidence,
                    'detected_objects': [divisions],
                    'visualization':
                    self.create_calibration_visualization(
                        image, [divisions], 'micrometer')
                })
            ...
            if circles is not None and len(circles) > 0 and known_length_um:
                ...
                circle_confidence = min(
                    num_circles / 5.0,
                    1.0)  # Better if multiple similar circles
                size_confidence = min(largest_circle[2] / 50.0,
                                      1.0)  # Better if reasonably sized
                confidence = (circle_confidence * 0.4 + size_confidence *
                              0.6) * 0.7  # Cap at 0.7 for circles
                ...
                results.update({
                    ...
                    'visualization':
                    self.create_calibration_visualization(
                        image, circles, 'circular')
                })

        return results

    def create_calibration_visualization(self, image, detected_objects,
                                         detection_type):
        ...
        if detection_type == 'scale_bar':
            ...
            for divisions_obj in detected_objects:
                if isinstance(divisions_obj,
                              dict) and 'tick_marks' in divisions_obj:
                    ...
                    for i, tick in enumerate(tick_marks):
                        ...
                        cv2.line(vis_image, (x1, y1), (x2, y2), (0, 255, 255),
                                 2)  # Yellow lines
                    ...
                    for i in range(min(3, len(tick_marks) - 1)):
                        ...
                        mid_y = min(int(tick1['center_y']),
                                    int(tick2['center_y'])) - 20

                        cv2.line(vis_image, (start_x, mid_y), (end_x, mid_y),
                                 (255, 255, 0), 1)  # Cyan spacing line
    ...
    def manual_calibration(self, image, point1, point2, known_length_um):
        ...
        distance_pixels = np.sqrt((point2[0] - point1[0])**2 +
                                  (point2[1] - point1[1])**2)
        ...
        vis_image = image.copy() if len(image.shape) == 3 else cv2.cvtColor(
        ...
        cv2.putText(
            vis_image, f"{distance_pixels:.1f}px = {known_length_um}μm",
    ...
    def validate_calibration(self, pixel_scale, image_context="microscopy"):
        ...
        reasonable_ranges = {
            ...
            "microscopy_medium":
            (2.0, 10.0),  # Medium magnification: 2-10 pixels/μm  
            "microscopy_high":
            (10.0, 50.0),  # High magnification: 10-50 pixels/μm
        ...
        return {
            ...
            pixel_scale,
            'expected_range':
            reasonable_ranges[image_context],
            'warning':
            None if is_valid else
            f"Scale seems unusually {'high' if pixel_scale > max_scale else 'low'} for {image_context}"
        }
